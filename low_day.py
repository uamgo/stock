...
class StockA:
    ...
    def filter_by_day(self, code, day_delta):
        ...
        start = self.time_fmt(day_delta + 20)
        ...
        if num_of_rows < day_delta:
            print(f"""No enough days: {num_of_rows} < {day_delta}, code={code}""")
            return False
        last_1 = num_of_rows - 1
        last_2 = num_of_rows - 2
        last_3 = num_of_rows - 3
        avg20 = day_df['收盘'].rolling(20).mean()
        if day_df['成交量'][last_1] < day_df['成交量'][last_2] \
                and day_df['成交量'][last_2] < day_df['成交量'][last_3]\
                and day_df['收盘'][last_1] > avg20[last_1]:
            # last_close = day_df['收盘'][last_1]
            # print(f"""code={code}, avg20={avg20[last_1]},  last_close={last_close}""")
            return True
        return False
    ...
    def filter_by_min(self, code):
        ...
        avg = sum_close/cnt
        if max_m_q < max_q * 0.5 and close_price > avg and cnt_hight > 150:
            return True
    ...
    def run_thread_check(self, codes):
        ...
        for row in codes:
            ...
            if self.filter_by_day(code, 20) and self.filter_by_min(code):
